...
pub fn draw(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let header_cells = build_header(is_wide, is_medium);
    ...
    let rows: Vec<Row> = app
        ...
        .map(|(i, entry)| {
            ...
            let exposure_indicator = match &entry.bind_address {
                ...
                BindAddress::Specific(_) => Span::styled("●", Style::default().fg(Color::Yellow)),
            };
    ...
    let title = format!(" Ports ({}) ", app.ports.len());
...
fn build_header(is_wide: bool, is_medium: bool) -> Vec<&'static str> {
    let mut h = vec!["PORT", "PROCESS", "TECH"];
    if is_medium {
        h.push("DIRECTORY");
        h.push("UPTIME");
    }
